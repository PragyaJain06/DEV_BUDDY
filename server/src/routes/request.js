...
requestRouter.post("/:status/:toUser", userAuth, async (req, res) => {
  ...
  sendEmail(presentToUser.email, "Someone is interested in you!", `${req.user.firstName} ${req.user.lastName} is interested in you!`)
...
if (status === "accepted") {
  ...
  sendEmail(
    ...
    .then(result => console.log("Accepted email result:", result))
    .catch(err => console.error("Failed to send accepted email:", err.message));
}
